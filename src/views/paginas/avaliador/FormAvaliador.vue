                        ...
                        <Column  headerStyle="min-width:10rem;">
                            <template #body="slotProps">
                                ...
                                <span v-else>
                                    ...
                                    <Button icon="pi pi-trash" class="p-button-rounded p-button-danger mt-2" @click="confirmDeleteAvaliador(slotProps.data)" />
                                </span>
                            </template>
                        </Column> 
    ...
    methods: { 
        ...
        async salvarAvaliador(){
            ...
            if(data.id === undefined){
                const req = await fetch("https://jose2550.c41.integrator.host/backend/avaliador", { // Enviando a requisição
                    method: "POST",
        ...
        async editarAvaliador(avaliador){
            ...
            const req = await fetch('https://jose2550.c41.integrator.host/backend/avaliador/'+avaliador.id);
            if(req.status == 200){
                const data = await req.json();
                this.avaliador = data;
        ...
        async deleteAvaliador(){
            const req = await fetch("https://jose2550.c41.integrator.host/backend/avaliador/delete/"+this.avaliador.id, { // Enviando a requisição
                method: "DELETE",
                headers:{ "Content-Type": "application/json" }
            }); 
            if(req.status == 204){
