    ...
    methods: { 
        ...
        async salvarAutor(){
            ...
            if(data.id === undefined){
                const req = await fetch("http://localhost:8080/autor", { // Enviando a requisição
                    method: "POST",
        ...
        async editarAutor(autor){
            ...
            const req = await fetch('http://localhost:8080/autor/'+autor.id);
            if(req.status == 200){
                const data = await req.json();
                this.autor = data;
        ...
        async deleteAutor(){
            const req = await fetch("http://localhost:8080/autor/delete/"+this.autor.id, { // Enviando a requisição
                method: "DELETE",
                headers:{ "Content-Type": "application/json" }
            }); 
            if(req.status == 204){
